...
def plot_3d(weld_type, origin_points, dir, rot_points, x, y, filter_thres):
    ...
    # Sobel 필터 적용
    sobel_x = cv2.Sobel(image_smooth, cv2.CV_64F, 1, 0, ksize=3)  # 수평
    ...
    ax2.imshow(sobel_edges, cmap='gray')        # aspect='auto' x, y 축 균등
    ax2.set_title('Sobel Filter')
    # ax2.axis('off')  # x, y 축을 숨깁니다.

    # 필터 적용 결과 좌표 표시 (원본 데이터 위에 표시)
    for i in range(len(x)):
        if sobel_edges[i][0] > np.max(sobel_edges) * filter_thres:  # 필터 결과가 특정 값 이상인 경우
...
# Main pipeline
def process_3d_data(weld_type, points, n_degree, dir, filter_thres=0.3):
    # 원래 포인트 복사
...
if __name__ == "__main__":
    ...
    # main 함수 호출
    process_3d_data(weld_type, points, degree, axis_dir, filter_thres)
